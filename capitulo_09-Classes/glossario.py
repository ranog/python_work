...
glossario['comprehension'] = 'Uma list comprehension combina o laço for e a criação de novos elementos em uma linha, e concatena cada novo elemento automaticglossa (Ex: squares = [value**2 for value in range(1,11)]).'
glossario['[:]'] = 'Para copiar uma lista, podemos criar uma fatia que incliglossario original inteira omitindo o primeiro e o segundo índices ( [:] ). Isso diz a Python para criar uma lista que começa no primeiro item e termina no último, gerando uma cópia da lista toda.'
...
glossario['rstrip'] = 'Garante que não haja espaços em branco do ladoglossari de uma string.'
glossario['str'] = 'Essa função diz a Python para representar valores que não  são strings como strings.'
